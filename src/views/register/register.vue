<template>
  <div class="register">
    ...
    <div class="protocol">
      ...
      <div class="desc">
        <span>我已阅读并同意<span style="color: #E6A913;" @click="showPopup=true">《财联邦理财服务平台使用协议》</span></span>
      </div>
    </div>
      ...
      showPlugin () {
        ...
        _this.checkWXAccount().then(response=>{
          if (response) {
                  ...
                  _this.checkWXAccount('change').then(response=>{
                    if(response){
                      ...
                      if(location.hash.split('=')[1]){
                        _this.$router.replace('share2C')
                      }else{
                        _this.$router.replace(window.localStorage.page);
                      }
                    }else{
            ...
            }else if(_this.displayText ==='请重新获取验证码!'||_this.displayText ==='短信验证码错误!'){
              _this.showToast = true;
            }else{
              if(location.hash.split('=')[1]){
                _this.userInfo.page = location.hash.split('=')[1].split('&')[0];
                _this.userInfo.userId = location.hash.split('=')[2];
              }
              console.log('userInfo'+this.userInfo);
              if(this.$route.params.from === 'independent'){  //从注册页来的
                _this.$router.replace({name:'setAccount',query:{accountInfo:_this.userInfo},params:{from:this.$route.params.from}});
              }else{
                _this.$router.replace({name:'setAccount',query:{accountInfo:_this.userInfo},params:{from:'scan'}});
              }
            }
          }
        });
      ...
      show:function(value){
        if(value){
          ...
          setTimeout(function(){
            ...
            self.showDrag = false;
            if(location.hash.split('=')[1]){
              self.$router.replace('share2C')
            }else{
              self.$router.replace(window.localStorage.page);
            }
          },3000);
        }
      }
