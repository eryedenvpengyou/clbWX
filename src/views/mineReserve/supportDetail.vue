<template>
  <div v-if="edit">
    ...
    <div class="trainApply" v-if="cost">
      ...
      <div class="whole_new">
        <div class="left"><span style="color:#f63636 ">*</span>培训开始时间</div>
        <group>
          <datetime v-model="data.costStartDate" format="YYYY-MM-DD HH:mm" :title="start" :start-date="dateTime" :min-hour="hours" placeholder="请选择"></datetime>
        </group>
      </div>
      ...
      <div class="whole_new">
        <div class="left"><span style="color:#f63636 ">*</span>培训结束时间</div>
        <group>
          <datetime v-model="data.costEndDate" format="YYYY-MM-DD HH:mm" :title="start" :start-date="dateTime" :min-hour="hours" placeholder="请选择"></datetime>
        </group>
      </div>
    ...
    <div class="trainApply" v-if="guest">
      ...
      <div class="whole_new">
        ...
        <group>
          <datetime v-model="data.guestDate" format="YYYY-MM-DD HH:mm" :title="start" :start-date="dateTime" :min-hour="hours" placeholder="请选择"></datetime>
        </group>
      </div>
  ...
  <div v-else-if="unEdit">
    ...
    <div v-for="(item,index) in data.teacherList" v-if="data.teacherList.length>0">
      ...
      <div v-show="data.status !== '已支付'">
        <div class="btnClass" v-if="data.status!=='支付成功'&&data.status!=='申请成功'&& data.status!=='已支付'&&data.status!=='待支付'" @click="cancel">{{btn}}</div>
        <div style="display: flex;display: -webkit-flex" v-if="data.status!=='支付成功' && data.status!=='申请成功'&&data.status==='待支付'">
    ...
    <div class="trainApply" v-if="cost">
      ...
      <div v-show="data.status !== '已支付'">
        <div class="btnClass" v-if="data.status!=='支付成功'&&data.status!=='申请成功'&&data.status!=='待支付'" @click="cancel">{{btn}}</div>
        <div style="display: flex;display: -webkit-flex" v-if="data.status!=='支付成功'&&data.status!=='申请成功'&&data.status==='待支付'">
          <div class="btnClass1"  @click="cancel">{{btn}}</div>
          <div class="btnClass1"  @click="pay">{{btn1}}</div>
        </div>
...
<script type="text/ecmascript-6">
  ...
  mounted(){
    var vm = this;
    var supportId = vm.$route.query.orderId?vm.$route.query.orderId:vm.$route.query.supportId;
