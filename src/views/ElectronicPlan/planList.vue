<template>
  <div class="planList">
    ...
    <scroller v-model="scrollerStatus" height="-95" lock-x scrollbar-y ref="scroller" :bounce="isbounce"
      ...
      <div>
        <div class="detail" v-for="(item,index) in detailList">
          ...
          <div class="person" v-if="item.status!=='已完成'"><span>状态：</span><span :class="item.class">{{item.status}}</span></div>
          <div class="person" v-if="item.status==='已完成'"><span>状态：</span><span :class="item.class" @click="reviewPdf(item.filePath, item.fileId)">点击查看</span></div>
    ...
    <c-pdf  @closepdf="closePdf" v-if="isshowpdf" @pdferr="pdfError" :pdfurl="testpdfurl"></c-pdf>
  </div>
...
<script type="text/ecmascript-6">
  ...
  export default {
    ...
    mounted(){
        ...
        if(window.localStorage.isApp ==='Y'){
          ...
          this.$post(fetchurl, {})
            ...
            vm.userIdValue = vm.appUserInfo.userId;
          }
        })
        }else{
          vm.userIdValue = JSON.parse(window.localStorage.userInfo).user.userId;
        }
    ...
    methods:{
      reviewPdf(filePath, fileId){
        let _this = this;
        _this.isshowpdf = true;
//        _this.testpdfurl="https://clbpicdev.oss-cn-shenzhen.aliyuncs.com/"+filePath;
        _this.testpdfurl = _this.config.baseUrl+'/api/fms/sys/attach/file/detail?fileId='+fileId+'&access_token='+window.localStorage.wxToken;

      },
      closePdf(){
        let _this = this;
        _this.isshowpdf = false;
      },
      pdfError(errurl){
        console.log(errurl);
      },
    ...
    ,
      goPlan(){
        ...
        vm.$post('/api/plan/queryAmount', {"userId": vm.userIdValue}).then(
          function (response) {
            if(response.rows[0].avilAmount > 0){
              vm.$router.push({name: 'addCompare', query: {page: 'normal'}});
            }else{
              vm.$vux.toast.show({text:'计划书额度不足'});
            }
          }, function (response) {
            console.log('接口出错了')
          });
