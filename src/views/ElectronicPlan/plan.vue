  ...
  <div class="plan">
    ...
    <div class="cardList">
      ...
      <div class="information">
        ...
        <span>{{yearPermium?yearPermium:'-'}}</span>
      </div>
      <div class="information">
        <div class="label">
          <span>保险金额：</span>
    ...
    <div class="cardList" v-if="isShowMoneyList">
      ...
      <div class="money">
        <span>身故保证倍偿金额</span>
        <span style="float:right">{{dieMoney1.dieDeposit}}{{currency}}</span>
      </div>
      <div class="money">
        <span>身故预期赔偿总金额</span>
        <span style="float:right">{{dieMoney1.dieCash}}{{currency}}</span>
      </div>
      <div class="money">
        <span>退保保证返回金额</span>
        <span style="float:right">{{dieMoney1.surrenderDeposit}}{{currency}}</span>
      </div>
      <div class="money">
        <span>退保预期返回金额</span>
        <span style="float:right">{{dieMoney1.surrenderCash}}{{currency}}</span>
      </div>
      <group>
        <range v-model="data9" :min="minAge" :range-bar-height="4" @on-change="changeAge(dieMoney,data9)"></range>
      </group>
...
<script type="text/ecmascript-6">
  ...
  export default {
      ...
      if(window.localStorage.isApp ==='Y'){
          ...
          vm.$post(fetchurl, {})
            ...
            if (response.success == true) {
              vm.appUserInfo = response.rows[0];
              trueUrl=JSON.stringify(vm.$store.state.productInfo)+"&planParams="+vm.$store.state.passParam.itemName+"&local="+localStorage.wxToken+"&user="+vm.appUserInfo.userName+'&phoneCode='+vm.appUserInfo.phoneCode+"&phone="+vm.appUserInfo.phone;
            }
        })
      }else{
    ...
    var linkUrl='https://weixin.fortunefed.com/#/plan?baseInfo='+trueUrl;
        ...
        if(window.localStorage.isApp ==='Y'){     //如果是app需要重新取一下个人信息
          ...
          vm.$post(fetchurl, {})
            .then(response=> {
              if (response.success == true) {
                 vm.appUserInfo = response.rows[0];
                 vm.user       = vm.appUserInfo.userName;
                 vm.phone      = 'tel:'+vm.appUserInfo.phoneCode.toString()+vm.appUserInfo.phone.toString();
                 vm.phoneNumber= vm.appUserInfo.phone;
              }
            })
        }
      }else{
  ...
  methods:{
    ...
    getPersonalPhoto(){
      ...
      if (window.localStorage.isApp === 'Y') {
        ...
        _this.$post('/api/channel/personal/query',{channelId:channelIdValue})
        .then(response=>{
          if(response.success){
...
//      if(window.localStorage.isApp === 'Y'){
//        alert(JSON.stringify(vm.baseInfo));
//      }
...
//              "amountType":_this.baseInfo.amountType,
            ...
            _this.$post('/api/public/production/queryCash', data).then(function (response) {
              if(response.rows.length>0){
                _this.isShowMoneyList = true;
                _this.dieMoney = response.rows;
              }
            }, function (response) {
