  ...
  <div class="mineOrder">
    <!--<sticky   :check-sticky-support="false">-->
      <tab  active-color="#e6a914" v-if="showTab"  :line-width="2" :line-height="2"  class="hb" custom-bar-width="35px" style="position:fixed;z-index:20">
        ...
        <tab-item @on-item-click="onItemClick" class="hb_tab" >转介绍</tab-item>
      </tab>
    <!--</sticky>-->
...
methods:{
  ...
  onItemClick(index){
    ...
    if(index===2){
      ...
      vm.$post('/api/ordOrder/queryReferral?page=1&pageSize=20',
        vm.screen1
      ).then(function(response){
        vm.$vux.loading.hide();
        if(response.total===0){
  ...
  loadMore(){
    ...
    if (vm.detailList.length === vm.total) {
      ...
      go(index){
        ...
        if(vm.screenIndex===2){
          vm.$router.push({path: '/orderDetail',query:{orderId:vm.detailList[index].orderId,type:'referral'}});
        }
      },
  choose(){
    var vm = this;
