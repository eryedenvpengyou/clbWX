<template>
  <div class="orderDetail">
    ...
    <div class="product" v-if="showCompany">
      ...
      <div class="other"><span>首期主险保费：</span><span>{{orderList.yearPayAmount}}</span></div>
      <div class="other"><span>保监征费：</span><span>{{orderList.iaLevies}}</span></div>
    ...
    <div class="customer" v-if="showCustomer">
      ...
      <div v-for="(item,index) in insurantList">
        ...
        <div class="other"><span>教育程度：</span><span>{{item.educationDesc}}</span></div>
        ...
        <div class="other"><span>身份证详细地址：</span><span>{{item.identityAddress}}</span>
        </div>
        <div class="other"><span>是否以身份证作为邮寄通讯地址：</span><span>{{item.identityFlag}}</span></div>
        ...
        <div class="other"><span>公司详细地址：</span><span>{{item.companyAddress}}</span></div>
        <div class="other"><span>职位：</span><span>{{item.position}}</span></div>
        ...
        <div class="other" v-if="item.badFlag==='是'"><span>保险公司/产品名称：</span><span>{{item.badInsuranceName}}</span></div>
        <div class="other" v-if="item.badFlag==='是'"><span>拒保推迟承保或者出现增加保费的原因：</span><span>{{item.badInsuranceType}}</span></div>
        <div class="other" v-if="item.badFlag==='是'"><span>最后一次申请日期：</span><span>{{item.badEffactiveDate}}</span></div>
        <div class="other"><span class="left1">您是否因意外、伤病或健康理由而申请社会福利或社会赔偿?</span><span class="right1">{{item.compensateFlag}}</span></div>
        <div class="other" v-if="item.compensateFlag==='是'"><span>保单1（请注明保险公司/险种/保额/保单生效日）：</span><span>{{item.compensateInsuranceName}}</span></div>
        <div class="other" v-if="item.compensateFlag==='是'"><span>保单2（请注明保险公司/险种/保额/保单生效日）：</span><span>{{item.compensateInsuranceType}}</span></div>
        <div class="other" v-if="item.compensateFlag==='是'"><span>保单3（请注明保险公司/险种/保额/保单生效日）：</span><span>{{item.compensateEffactiveDate}}</span></div>

        <div class="other"><span class="left1">您是否有现行生效（包括在申请中）之保险保障?</span><span class="right1">{{item.guaranteeFlag}}</span></div>
        <div class="other" v-if="item.guaranteeFlag==='是'"><span>承保保险公司：</span><span>{{item.guaranteeInsuranceName}}</span></div>
        <div class="other" v-if="item.guaranteeFlag==='是'"><span>保险种类：</span><span>{{item.guaranteeInsuranceType}}</span></div>
        <div class="other" v-if="item.guaranteeFlag==='是'"><span>保险生效年月：</span><span>{{item.guaranteeEffactiveDate}}</span></div>

        <div class="other"><span class="">美国连接性证明：</span><span class="">{{item.usaConnectFlag}}</span></div>
      ...
      <div v-for="(item,index) in applicantList" v-if="showTwo">
        ...
        <div class="other"><span class="left1">您是否有现行生效（包括在申请中）之保险保障?</span><span class="right1">{{item.guaranteeFlag}}</span></div>
        <div class="other" v-if="item.guaranteeFlag==='是'"><span>保单1（请注明保险公司/险种/保额/保单生效日）：</span><span>{{item.guaranteeInsuranceName}}</span></div>
        <div class="other" v-if="item.guaranteeFlag==='是'"><span>保单2（请注明保险公司/险种/保额/保单生效日）：</span><span>{{item.guaranteeInsuranceType}}</span></div>
        <div class="other" v-if="item.guaranteeFlag==='是'"><span>保单3（请注明保险公司/险种/保额/保单生效日）：</span><span>{{item.guaranteeEffactiveDate}}</span></div>

        <div class="other" style="display: flex;justify-content: space-between;"><span class="">美国连接性证明：</span><span class="" style="min-width: 20%; text-align: center">{{item.usaConnectFlag}}</span></div>
  ...
  mounted(){
    ...
    vm.type=vm.$route.query.type;
      vm.$post('/api/clb/common/clbCode', {
        ...
        vm.usaConnectivityProof = JSON.parse(response).usaConnectivityProof;
    ...
    function second(){
      vm.$post('/api/ordOrder/query', {
        ...
        let firstPayMethodList = vm.orderList.firstPaymentMethod.split(',');
        let firstPayMethodListStr = []
        firstPayMethodList.map(item => {
          vm.firstPayMethodList.map(i => { //第一次支付方式
            if (i.value == item) {
              firstPayMethodListStr.push(i.meaning);
            }
          })
        })
        vm.orderList.firstPaymentMethod = firstPayMethodListStr.toString();

        let renewalPayMethodList = vm.orderList.renewalPaymentMethod.split(',');
        let renewalPayMethodListStr = [];
        renewalPayMethodList.map(item => {
          vm.renewalPayMethodList.map(i => { // 续期支付
            if (i.value == item) {
              renewalPayMethodListStr.push(i.meaning);
            }
          })
        })
        vm.orderList.renewalPaymentMethod = renewalPayMethodListStr.toString();
    ...
    function fifth(){
      vm.$post('/api/ordCustomer/query', {
        ...
        console.debug("APPLICANT")
        console.debug(JSON.stringify(vm.applicantList));
        ...
        for (let i in vm.applicantList) {
          ...
          let date;
          if(vm.applicantList[i].certificateEffectiveDate != null && vm.applicantList[i].certificateEffectiveDate != ""){
            date = new Date(vm.applicantList[i].certificateEffectiveDate.replace(/-/g,'/'));
          ...
          if(vm.applicantList[i].hasOwnProperty('guaranteeFlag')) {
            vm.applicantList[i].guaranteeFlag  = vm.applicantList[i].guaranteeFlag  === 'Y' ? '是':'否';
//            vm.applicantList[i].usaConnectFlag = vm.applicantList[i].usaConnectFlag === 'Y' ? '是':'否';
          }else{
            vm.applicantList[i].guaranteeFlag  =  '是';
//            vm.applicantList[i].usaConnectFlag =  '是';
          }
          ...
          if(vm.applicantList[i].premiumRate != null && vm.applicantList[i].premiumRate != ''){
            vm.applicantList[i].premiumRate=Number((vm.applicantList[i].premiumRate||0)*100).toFixed(2)+'%';
          }

        }

        vm.applicantList.map(item => {
          vm.usaConnectivityProof.map(flag => {
            if (item.usaConnectFlag === flag.value) {
              item.usaConnectFlag = flag.meaning;
            }
          })
        })
    ...
    function sixth(){
      vm.$post('/api/ordCustomer/query', {
        ...
        console.debug("INSURANT")
        console.debug(JSON.stringify(vm.insurantList));
        ...
        for (let i in vm.insurantList) {
          ...
          let date;
          if(vm.insurantList[i].certificateEffectiveDate != null && vm.insurantList[i].certificateEffectiveDate != ""){
            date = new Date(vm.insurantList[i].certificateEffectiveDate.replace(/-/g,'/'));
          ...
          if(vm.insurantList[i].hasOwnProperty('guaranteeFlag')) {
            vm.insurantList[i].guaranteeFlag  = vm.insurantList[i].guaranteeFlag  === 'Y' ? '是':'否';
//            vm.insurantList[i].usaConnectFlag = vm.insurantList[i].usaConnectFlag === 'Y' ? '是':'否';
          }else{
            vm.insurantList[i].guaranteeFlag  =  '是';
//            vm.insurantList[i].usaConnectFlag =  '是';
          }
          ...
          vm.insurantList[i].premiumRate=Number((vm.insurantList[i].premiumRate || 0)*100).toFixed(2)+'%';


        }

        vm.insurantList.map(item => {
          vm.usaConnectivityProof.map(flag => {
            if (item.usaConnectFlag === flag.value) {
              item.usaConnectFlag = flag.meaning;
            }
