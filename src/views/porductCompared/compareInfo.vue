  ...
  <div class="compareInfo">
    ...
    <div class="compareTable">
      ...
      <div class="compare">
        ...
        <div class="productOne">{{compare1Info.yearPermium?compare1Info.yearPermium:'-'}}</div>
        <div class="productTwo">{{compare2Info.yearPermium?compare2Info.yearPermium:'-'}}</div>
      </div>
      <div class="compare">
        <div class="tableTitle">保费总额</div>
        <div class="productOne">{{baseInfo[0].productionAgeLimit * compare1Info.yearPermium?parseFloat(baseInfo[0].productionAgeLimit * compare1Info.yearPermium).toFixed(2):'-'}}</div>
        <div class="productTwo">{{baseInfo[1].productionAgeLimit * compare2Info.yearPermium?parseFloat(baseInfo[1].productionAgeLimit * compare2Info.yearPermium).toFixed(2):'-'}}</div>
      </div>
    </div>
    <div class="compareTable">
    ...
    <div class="cardList" v-if="isShowMoneyList">
      <group>
        <range v-model="data9" :min="minAge" :range-bar-height="4" @on-change="changeAge(dieMoneys1,dieMoneys2,data9)"></range>
      </group>
      <div class="remark">
    ...
    <div class="compareTable" v-if="isShowMoneyList">
      <div class="compare">
        <div class="tableTitle">重疾/身故理赔</div>
        <div class="productOne">{{dieMoney1.dieDeposit?dieMoney1.dieDeposit:'-'}}</div>
        <div class="productTwo">{{dieMoney2.dieDeposit?dieMoney2.dieDeposit:'-'}}</div>
      </div>
      <div class="compare">
        <div class="tableTitle">身故预期赔偿</div>
        <div class="productOne">{{dieMoney1.dieCash?dieMoney1.dieCash:'-'}}</div>
        <div class="productTwo">{{dieMoney2.dieCash?dieMoney2.dieCash:'-'}}</div>
      </div>
      <div class="compare">
        <div class="tableTitle">退保价值</div>
        <div class="productOne">{{dieMoney1.surrenderDeposit?dieMoney1.surrenderDeposit:'-'}}</div>
        <div class="productTwo">{{dieMoney2.surrenderDeposit?dieMoney2.surrenderDeposit:'-'}}</div>
      </div>
      <div class="compare">
        <div class="tableTitle">退保预期返回</div>
        <div class="productOne">{{dieMoney1.surrenderCash?dieMoney1.surrenderCash:'-'}}</div>
        <div class="productTwo">{{dieMoney2.surrenderCash?dieMoney2.surrenderCash:'-'}}</div>
      </div>
    </div>
  </div>
</template>
...
<script type="text/ecmascript-6">
  ...
  export default {
    ...
    if(!isEmptyObject(_this.baseInfo)&&!isEmptyObject(_this.baseInfo1)){
      ...
      _this.getAmountList(this.baseInfo[0].itemId,1,this.baseInfo[0].productionAgeLimit).then(response=>{
        _this.getAmountList(this.baseInfo[1].itemId,2,this.baseInfo[1].productionAgeLimit);
      });
    },
    methods: {
      ...
      changeAge(array1,array2,data){
        let _this = this;
        if(_this.data9 === data) {
          array1.forEach(value=> {
            if ((data-_this.minAge) === value.insuranceYear) {
              _this.dieMoney1 = value;
            }
          });
          array2.forEach(value=> {
            if ((data-_this.minAge) === value.insuranceYear) {
              _this.dieMoney2 = value;
            }
          })
        }
      },
      getAmountList(itemId,productType,productionAgeLimit){
        return new Promise(resolve=>{
          var vm = this;
          let data = [{
            "age": vm.minAge,
            "coverage": vm.baseInfo[0].coverage,
            "currency": vm.baseInfo.currency,
            "gender": vm.baseInfo[0].gender,
            "itemId": itemId,
            "nextAge": vm.data9,
            "paymentMethod": vm.baseInfo[0].payMethod,
            "productionAgeLimit": productionAgeLimit,
            "smokeFlag": vm.baseInfo[0].smokeFlag,
            //        "amountType":vm.baseInfo.amountType,
          }];
          vm.$post('/api/public/production/queryCash', data).then(function (response) {
            if(response.rows.length>0){
              vm.isShowMoneyList = true;
              if(productType ===1){
                vm.dieMoneys1 = response.rows;
              }else{
                vm.dieMoneys2 = response.rows;
              }
        ...
        _this.$post(url,param).then(response=>{
          if(response.success){
            response.rows[0].prdAttributeSetLine.forEach((value,index)=>{
              ...
              if(value.attName === "是否附加险"){
                console.debug(response.rows[0][value.fieldCode.toLowerCase()]);
                if(response.rows[0][value.fieldCode.toLowerCase()] === "N"){
                  response.rows[0][value.fieldCode.toLowerCase()] = "否";
                }else if(response.rows[0][value.fieldCode.toLowerCase()] === "Y"){
                  response.rows[0][value.fieldCode.toLowerCase()] = "是";
                }
              }
            ...
            response.rows[1].prdAttributeSetLine.forEach((value,index)=>{
              if(value.attName === "是否附加险"){
                if(response.rows[1][value.fieldCode.toLowerCase()] === "N"){
                  response.rows[1][value.fieldCode.toLowerCase()] = "否";
                }else if(response.rows[1][value.fieldCode.toLowerCase()] === "Y"){
                  response.rows[1][value.fieldCode.toLowerCase()] = "是";
                }
              }
