    ...
    created(){
      ...
      this.getCompanyList().then(response=>{
        this.companyList.map(item => {
          this.getPicture(item.fileId).then(data => {
//            item.img = data ? data : this.imgPath
            this.$set(item, 'img', data || this.imgPath)
          })
        })
        this.getCLBCode();
      })
    ...
    methods:{
      ...
      goPlan(item,page){
        ...
        if(page ==='normal'){
          ...
          router = 'insurance';
        }else if(page ==='budge' || page ==='budget'){
          router = 'budget'
        }else if(page ==='planList'){
          router = 'planList'
        }
      ...
      getItem:function(item){
        let type = this.$route.query.hasOwnProperty('type')?this.$route.query.type:'';
        ...
        if(location.hash.split('=')[1] ==='normal'|| location.hash.split('=')[1] ==='planList' ||location.hash.split('=')[1] ==='plan'||location.hash.split('=')[1] ==='budge'||location.hash.split('=')[1] ==='budget'){
...
//                return x.midClassName!=='附加险'&&x.attribute1 !=='Y';
      ...
      getCompanyList(){
        return new Promise((resolve)=>{
          let _this = this;
          _this.$vux.loading.show({text:'loading'});
          let  url = `/api/supplier/queryAll?page=1&pagesize=10`,
            params = {
              page:1,
              pagesize:10,
              businessScope: "保险",
