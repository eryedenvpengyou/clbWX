<template>
  <div class="product_center">
    ...
    <!-- 保险页面-->
    <div>
    <scroller v-model="scrollerStatus"
              height="-45"
              style="margin-top:45px" lock-x scrollbar-y
              ref="scroller"
              :bounce="isbounce"
              :use-pullup="showUp"
              :pullup-config="upobj"
    ...
    vm.$post('/api/clb/common/clbCode', {
      ...
      vm.$post('/api/production/headerlist',
        vm.screen1
          ...
          vm.dataList.map((item, index) => {
            if (item.pictureFilePath != null) {
              item.pictureFilePath = vm.$store.getters.getPictureUrl + item.pictureFilePath;
            } else {
              item.pictureFilePath = vm.examplePic;
            }
            item.attribute120 = '';
            item.currencyCode = '';
            item.prdItemPaymode.map(prdItem => {
              vm.currencyList.map(currency => {
                if (prdItem.currencyCode === currency.value) {
                  prdItem.currency = currency.meaing;
                }
              });
              if(prdItem.currency !=null){
                item.currencyCode += prdItem.currency +'、' ;
              }
            });
            item.prdItemSublineList.map(subLineItem=>{
              let n = Number(subLineItem.sublineItemName);
              if(!isNaN(n)){
                subLineItem.sublineItemName+='年';
              }
              item.attribute120 += subLineItem.sublineItemName + '、';
            })
          });
