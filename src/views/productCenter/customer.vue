<template>
  <div class="customer">
    ...
    <div class="customer_search" v-if="showCustomer">
      ...
      <scroller v-model="scrollerStatus" height="-140" lock-x scrollbar-y ref="scroller" :bounce="isbounce"
...
//        string1: '',
    ...
    async mounted () {
      let response = await this.$post('/api/wxAccount/testGetOpenId', {})
      this.channelId = response.rows[0].relatedPartyId || JSON.parse(window.localStorage.userInfo).user.relatedPartyId
      let vm = this

      let policyList = vm.$store.getters.getPolicyList
