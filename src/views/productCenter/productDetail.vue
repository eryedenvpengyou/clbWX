  ...
  <div class="product">
    <div class="content">
      <div class="header">
        ...
        <div v-if="detail.midClassName !== '附加险'" class="btn" @click="compare">对比其他产品</div>
      </div>
      ...
      <div class="detail" v-for="item in attGroupList">
        <label style="letter-spacing: 2px;font-size: 14px!important;font-family: 'PingFang SC','微软雅黑',Helvetica, Tahoma, Arial, 'Hiragino Sans GB', 'Heiti SC', 'Microsoft YaHei', 'WenQuanYi Micro Hei', sans-serif!important;">{{item.attName}}：{{detail1[item.fieldCode.toLowerCase()]}}</label>
      </div>
    ...
    <div class="footer" >
      <div class="footer_detail">
        <div class="footer_detail_left" v-for="(item,index) in footerList" @click="goPage(index)" v-show="detail.midClassName === '附加险' ? index >= 3 : true">{{item.value}}</div>
      </div>
    </div>
  </div>
</template>
...
<script type="text/ecmascript-6">
  ...
  mounted(){
    ...
    vm.isAPP = window.localStorage.isApp === "Y"?"true":"false";

    if(vm.isAPP === 'true') {      //如果是app,不显示普通计划书
      vm.footerList.splice(0, 1);
    }
  ...
  methods:{
    judgeData(itemId){
      return new Promise((resolve,reject)=>{
        let _this = this;
        _this.$http('api/production/getPrdPremium',{'itemId':itemId})
          .then(response=>{
...
//           alert(error);
    ...
    goPage:function(index){
      var vm=this;
      index = window.localStorage.isApp === 'Y'?index + 1:index;
      if(index===3){
        ...
        if(index===2){     //保费测算
          vm.$post('/api/production/getPrdPremium',{'itemId':item.itemId})
            .then(response=>{
              if(response){
                if(response.rows.length > 0 ){
                  vm.$router.push({path:'/budget'});
                }else{
                  vm.$vux.toast.show({text:'抱歉，由于产品资料不全，无法进行保费测算！'});
                }
              }else{
                vm.$vux.toast.show({text:'抱歉，由于产品资料不全，无法进行保费测算！'});
              }
