...
//      path.push(vm.detail.prdItemSublineList[i].sublineItemName)
//    }
    vm.detail.prdItemSublineList.map(item => {
      path.push(item.sublineItemName);
    });
    vm.pathList = path;
    ...
    reservePath:function(newVal,oldVal){
      ...
      for(let i in vm.detail.prdItemSublineList){
        if(newVal===vm.detail.prdItemSublineList[i].sublineItemName){
          ...
          if(vm.price1!="实时报价"){
            vm.basePrice = vm.price1 == null?"":"￥"+ vm.price1;
          }else{
            vm.basePrice  = vm.price1 == null?"":vm.price1;
          }
        }
  ...
  ,
    onConfirm2:function(){
      ...
      var mon = date.getMonth()+1>9?date.getMonth()+1:'0'+date.getMonth(); //获取当前月份
      var da  = date.getDate()>9?date.getDate():'0'+date.getDate(); //获取当前日
      var ho  = date.getHours();
      var min = date.getMinutes();     //获取当前分钟数(0-59)
      let nowTime=year+'/'+mon+'/'+da+' '+ho+':'+min;
      ...
      }else if(vm.reserveTime < nowTime){
