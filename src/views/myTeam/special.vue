<template>
  <div class="special">
    ...
    <div class="footer">
      <img class="left" :src="left_1" v-if="index+1===1||total===1||total===0" />
      <img class="left" :src="left_2" v-if="index+1>1&&total>1" @click="reduce" />
      <div class="middle" v-if="total>0">{{index+1}}/{{total}}</div>
      <div class="middle" v-if="total===0">{{index}}/{{total}}</div>
      <img class="right" :src="right_1" v-if="total===1||index+1===total||total===0" />
...
<script type="text/ecmascript-6">
  export default {
    data(){
    return{
      screen:JSON.parse(this.$route.query.screen),
      role:[],
      index:0,
      total:0,
      total:null,
