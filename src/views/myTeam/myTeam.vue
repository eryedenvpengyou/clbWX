        ...
        <tab :line-width=2 active-color='#E6A914' custom-bar-width="60px" v-model="index">
          ...
          <div v-if="demo2 === '团队成员'" class="teamRemember">
            ...
            <div class="cardList" v-for="remember in teamRemember" @click="goPage('mineTeam',remember)">
              ...
              <span style="display: inline-block;overflow: hidden;text-overflow: ellipsis;white-space: nowrap;width: 100%;">佣金分成级别: &nbsp;{{remember.rate1}},{{remember.rate2}},{{remember.rate3}},{{remember.rate4}},{{remember.rate5}},{{remember.rate6}},{{remember.rate7}},{{remember.rate8}},{{remember.rate9}},{{remember.rate10}}</span>
    ...
    export default {
      ...
      methods: {
        checkData(value,type){
          if(type ==='phone'){
            if(this.optionPhone ==='86'&&value.length===11){
                return true;
            }else if(this.optionPhone ==='00852'&&value.length==8){
              return true;

            }else if(this.optionPhone ==='00853'&&value.length==8){
              return true;

            }else if(this.optionPhone ==='00886'&&value.length==9){
              return true;
            }else{
              ...
              if(response.success){
                response.rows.forEach((value,index)=>{
                  _this.getSomeParam(value.channelId).then(data=>{
                    value.rate1 =  data.rate1||data.rate1!==undefined?data.rate1*100+'%':'0%';
                    value.rate2 =  data.rate2||data.rate2!==undefined?data.rate2*100+'%':'0%';
                    value.rate3 =  data.rate3||data.rate3!==undefined?data.rate3*100+'%':'0%';
                    value.rate4 =  data.rate4||data.rate4!==undefined?data.rate4*100+'%':'0%';
                    value.rate5 =  data.rate5||data.rate5!==undefined?data.rate5*100+'%':'0%';
                    value.rate6 =  data.rate6||data.rate6!==undefined?data.rate6*100+'%':'0%';
                    value.rate7 =  data.rate7||data.rate7!==undefined?data.rate7*100+'%':'0%';
                    value.rate8 =  data.rate8||data.rate8!==undefined?data.rate8*100+'%':'0%';
                    value.rate9 =  data.rate9||data.rate9!==undefined?data.rate9*100+'%':'0%';
                    value.rate10 = data.rate10||data.rate10!==undefined?data.rate10*100+'%':'0%';
